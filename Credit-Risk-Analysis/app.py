...
def about():
    if request.method == "POST":
        ...
        married = int(married) if married else 0
        dependants = int(dependants) if dependants else 0
        education = int(education) if education else 0
        selfEmployed = int(selfEmployed) if selfEmployed else 0
        amountTerm = int(amountTerm) if amountTerm else 0
        credit = int(credit) if credit else 0
        parea = int(parea) if parea else 0
        # Log Transforming this values as we've done the same while model building
        loanAmount = np.log(int(loanAmount)) if loanAmount else 0
        totalIncome = np.log(int(totalIncome)) if totalIncome else 0
